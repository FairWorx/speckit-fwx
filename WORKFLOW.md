...
- The `speckit.fwx.mod.docs` command (see Phase 3) syncs any implementation changes back to the SDD, keeping it accurate without requiring perfect upfront design.
...
**Important:** Converge and `speckit.fwx.mod.docs` serve different purposes:
...
```
/speckit.fwx.mod.docs [module_name]
```
...
Phase 3: Documentation & Closure
─────────────────────────────────────────────
/speckit.fwx.mod.docs [name]            (required — sprint docs + SDD sync)
